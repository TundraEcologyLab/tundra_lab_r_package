...
#' belonging to an Annex or Farm site has any letters removed from its plot number. Column name
...
standardise_plot_number <- function(dataframe){
    ...
    dataframe <- dplyr::mutate(
        ...
        # their plot number are identified as an Annex_BS site
    ...
    # Remove all letters from plot numbers, except for Annex and Farm sites which justifiably contain
    # such characters
    dataframe <- dplyr::mutate(dataframe,
                               plot = ifelse(!grepl("annex|farm", .data[["site"]], ignore.case = TRUE)&
                                                 grepl("[A-Z]+", .data[["plot"]], ignore.case = TRUE),
                                             gsub("[A-Z]", "", .data[["plot"]], ignore.case = TRUE),
                                             .data[["plot"]]))
